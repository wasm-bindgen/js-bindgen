...
fn import() {
	super::test(
		...
		quote! {
			const _: () = {
				const ARR_0: [u8; 11] = *b"\x03\0foo\x03\0bar\0";
				const ARR_1: [u8; 7] = *b"baz\nqux";
...
fn required_embeds() {
	super::test(
		...
		quote! {
			const _: () = {
				const ARR_0: [u8; 11] = *b"\x03\0foo\x03\0bar\x01";
				const VAL_1: &str = "baz";
